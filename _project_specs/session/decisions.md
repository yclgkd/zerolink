...
**Decision**: Self-hosted `create_finish` now treats WebAuthn finalization as a verifier-backed boundary: it loads and deletes the stored `create` challenge before verification, maps missing/invalid challenges to `CHALLENGE_INVALID`, maps verifier failures to `ATTESTATION_UNVERIFIABLE`, and persists only the verifier-derived `StoredCredential` fields (`credentialId`, `publicKey`, `signCount`, `aaguid`, `transports`) instead of raw attestation blobs.
...
**Choice**: Add a Go-native verifier in `internal/webauthn`, consume the create challenge in a dedicated pre-commit step for WebAuthn mode, keep softkey/password create semantics unchanged, and store only the finalized credential material returned by the verifier.
...
**Trade-offs**: The self-hosted Go module now depends on CBOR decoding for attestation parsing, and the verifier still intentionally rejects x5c-backed certificate-chain attestations just like the current Worker implementation. WebAuthn challenge consumption is intentionally one-shot even when verification fails, so clients must restart `create_begin` after a bad attestation attempt.
