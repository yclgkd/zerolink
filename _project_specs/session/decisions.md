...
**Follow-up (2026-03-20)**: The upstream `googleapis/release-please-action@v4` still declares `runs: node20`, which now emits GitHub's Node 20 deprecation warning. ZeroLink therefore runs the official `release-please` CLI under `actions/setup-node@v6` (Node 24) and adds an explicit preflight failure message when `RELEASE_PLEASE_TOKEN` is missing.
