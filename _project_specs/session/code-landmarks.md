...
| `packages/backend/src/do/SecretVaultCompound.ts` | Durable Object compound-commit path — validates signed update/delete intents, enforces inline file ciphertext ceilings for `payloadKind: "file"`, and persists delivery proofs used by anchored decrypt verification |
...
| `packages/frontend/src/crypto/orchestrator-decrypt.ts` | Delivered-payload decrypt path — resolves anchored delivery proof versions, preserves backward-compatible raw-text decoding, and only treats undeclared payloads as files when the decrypted bytes form a valid file envelope |
...
| `packages/frontend/src/crypto/orchestrator-decrypt.ts` + `packages/shared/src/payload.ts` | `payloadKind` is optional by design | New deliveries sign and persist `payloadKind`, but decrypt must continue to accept undeclared historical payloads: strict file decoding only applies when `payloadKind === "file"`, while undeclared payloads may still decode a valid file envelope and otherwise fall back to raw text. |
