...
Remove `_project_specs/session/archive/` and collapse session tracking to the three live files only.

## Current Status

- **Phase**: Restructuring `_project_specs` layout on the documentation PR branch
- **Progress**: Deleting the archive layer, normalizing repo guidance to `current-state.md` / `decisions.md` / `code-landmarks.md`, and removing stale archive-path references from preserved session history.
- **Known Constraint**: Historical work detail now relies on git/PR history instead of a repo-local completed log, so the live session files need to stay concise and current.

## Latest Update (2026-03-11)

- `_project_specs/session/archive/README.md`, `_project_specs/session/archive/completed.md`, `.ai/workflows.md`, repo-local skill docs — Removed the temporary archive layer; `_project_specs/session/` now contains only `current-state.md`, `decisions.md`, and `code-landmarks.md`.
- `.agents/skills/base/SKILL.md`, `.agents/skills/session-management/SKILL.md`, `.claude/skills/base/SKILL.md`, `.claude/skills/session-management/SKILL.md` — Rewrote session-tracking guidance to use full checkpoints and handoffs instead of archive files or completed-history logs.
- `_project_specs/session/current-state.md`, `_project_specs/session/decisions.md`, `_project_specs/session/code-landmarks.md` — Logged the no-archive decision, removed stale archive landmarks, and normalized preserved history entries so they point only at live session files.
- Validation: `rg -n "session/archive|archive/|completed\\.md|Archive" .ai .agents .claude _project_specs/session/code-landmarks.md`, `git diff --check`
- `_project_specs/session/current-state.md`, `_project_specs/session/decisions.md` — Cleaned the remaining misleading historical wording after the docs pass: old `RK=required`, `admin_mode`, `security_profile`, and active-Changesets phrasing is now either corrected to the shipped field name or explicitly annotated as superseded history.
...
- `_project_specs/session/current-state.md`, `_project_specs/session/decisions.md`, `_project_specs/session/code-landmarks.md` — Clarified append-only timeline expectations, fixed `current-state.md` section ordering, and removed the stale `.changeset/config.json` landmark.
...
- `_project_specs/session/current-state.md`, `_project_specs/session/decisions.md` — Recorded the padding policy fix, the decision to keep the shared AES helper default at 4 KB, and the regression validation footprint for future sessions.
...
- `_project_specs/session/current-state.md`, `_project_specs/session/decisions.md` — Recorded the ManagePage password-prompt visibility rule and validation results for future sessions.
...
- `_project_specs/session/current-state.md`, `_project_specs/session/decisions.md` — Recorded the CI runtime migration, validation results, and the release-signing constraint for future sessions.
...
- `_project_specs/session/current-state.md`, `_project_specs/session/decisions.md` — Recorded the terminal-state share-link cleanup for future sessions.
...
- `_project_specs/session/current-state.md`, `_project_specs/session/decisions.md`, `_project_specs/session/code-landmarks.md` — Recorded the new sync/runtime-integrity behavior and validation results for future sessions.
...
- `_project_specs/session/decisions.md`, `_project_specs/session/code-landmarks.md` — Recorded the new “tests/build before merge” policy and the new CI entrypoint for future sessions.
...
- `_project_specs/session/decisions.md` / `_project_specs/session/current-state.md` — Replaced read-implies-channel-burn terminology with explicit delete/local-burn/expiry semantics
