...
- **Progress**: Main is merged with non-discoverable passkeys, and the remaining follow-up is aligning passkey-backed E2E coverage with the new Quick Share default by selecting Secure Share explicitly before create.
...
- `packages/frontend/e2e/happy-path.spec.ts`, `packages/frontend/e2e/terminal-state.spec.ts` — Updated passkey-backed E2E flows to select Secure Share explicitly before create so the tests match the new default Quick Share UX.
