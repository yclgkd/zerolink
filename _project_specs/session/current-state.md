...
Simplify `_project_specs` to a session-plus-archive structure and remove the empty `todos/` layer.

## Current Status

- **Phase**: Restructuring `_project_specs` layout on the documentation PR branch
- **Progress**: Moving completed history into `session/archive/`, deleting the empty `todos/` directory, and updating repo guidance so agents/readers follow the simplified structure.
- **Known Constraint**: Historical session snapshots remain intentionally preserved below, so path references are being normalized to the new archive location instead of rewriting the underlying implementation history.
...
- `_project_specs/session/current-state.md`, `_project_specs/session/decisions.md`, `_project_specs/session/archive/completed.md` — Cleaned the remaining misleading historical wording after the docs pass: old `RK=required`, `admin_mode`, `security_profile`, and active-Changesets phrasing is now either corrected to the shipped field name or explicitly annotated as superseded history.
...
- `_project_specs/session/current-state.md`, `_project_specs/session/decisions.md`, `_project_specs/session/archive/completed.md`, `_project_specs/session/code-landmarks.md` — Clarified append-only timeline expectations, fixed `current-state.md` section ordering, and removed the stale `.changeset/config.json` landmark.
...
- `_project_specs/session/current-state.md`, `_project_specs/session/decisions.md`, `_project_specs/session/archive/completed.md` — Recorded the padding policy fix, the decision to keep the shared AES helper default at 4 KB, and the regression validation footprint for future sessions.
...
- `_project_specs/session/current-state.md`, `_project_specs/session/decisions.md`, `_project_specs/session/archive/completed.md` — Recorded the ManagePage password-prompt visibility rule and validation results for future sessions.
...
- `_project_specs/session/current-state.md`, `_project_specs/session/decisions.md`, `_project_specs/session/archive/completed.md` — Recorded the CI runtime migration, validation results, and the release-signing constraint for future sessions.
...
- `_project_specs/session/current-state.md`, `_project_specs/session/archive/completed.md` — Recorded the terminal-state share-link cleanup for future sessions.
...
- `_project_specs/session/current-state.md`, `_project_specs/session/decisions.md`, `_project_specs/session/code-landmarks.md`, `_project_specs/session/archive/completed.md` — Recorded the new sync/runtime-integrity behavior and validation results for future sessions.
...
- `_project_specs/session/decisions.md`, `_project_specs/session/code-landmarks.md`, `_project_specs/session/archive/completed.md` — Recorded the new “tests/build before merge” policy and the new CI entrypoint for future sessions.
...
- `_project_specs/session/decisions.md` / `_project_specs/session/archive/*.md` — Replaced read-implies-channel-burn terminology with explicit delete/local-burn/expiry semantics
