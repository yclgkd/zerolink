...
func TestProtocolServiceMultipartReplacementRollbackKeepsPreviousUpload(t *testing.T) {
	db := openTestDatabase(t)
	resetTestTables(t, db)
	...
	commitFileDelivery := func(version int64, nonce string, ts int64, fileRef filestore.MultipartFileRef) error {
		t.Helper()
	...
	firstFileRef := filestore.MultipartFileRef{
		...
		TotalPlaintextBytes:  64,
		TotalCiphertextBytes: 80,
		BaseIV:               encodeBase64URL([]byte("base-iv-rollback-1")),
		EncContentKey:        encodeBase64URL([]byte("enc-key-rollback-1")),
		Chunks: []filestore.MultipartFileRefChunk{
			{
				Index:           0,
				StorageKey:      "files/upload-rollback-1/0000.bin",
				CiphertextBytes: 80,
	...
	secondFileRef := filestore.MultipartFileRef{
		...
		TotalPlaintextBytes:  96,
		TotalCiphertextBytes: 112,
		BaseIV:               encodeBase64URL([]byte("base-iv-rollback-2")),
		EncContentKey:        encodeBase64URL([]byte("enc-key-rollback-2")),
		Chunks: []filestore.MultipartFileRefChunk{
			{
				Index:           0,
				StorageKey:      "files/upload-rollback-2/0000.bin",
				CiphertextBytes: 112,
	...
	if err := commitFileDelivery(0, "nonce-manage-file-rollback-1", timestamp+2_000, firstFileRef); err != nil {
		t.Fatalf("first CompoundCommit() error = %v", err)
	}

	err := commitFileDelivery(1, "nonce-manage-file-rollback-1", timestamp+4_000, secondFileRef)
	requireProtocolError(t, err, "NONCE_REPLAY", 409)

	if len(cleaner.deleted) != 0 {
		t.Fatalf("deleted uploads = %d, want 0 when transaction rolls back", len(cleaner.deleted))
	}
	...
	if decryptFetch.FileRef == nil {
		t.Fatal("decryptFetch.FileRef = nil, want original multipart payload")
	}
	if decryptFetch.FileRef.Chunks[0].StorageKey != firstFileRef.Chunks[0].StorageKey {
		t.Fatalf(
			"decryptFetch.FileRef.Chunks[0].StorageKey = %q, want %q",
			decryptFetch.FileRef.Chunks[0].StorageKey,
			firstFileRef.Chunks[0].StorageKey,
		)
	}
}
