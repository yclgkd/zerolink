...
| `/api/file_policy` | `GET` | 无 | `FilePolicyResponseSchema` | `apiClient.filePolicy()` | 返回部署侧文件上限、历史 inline envelope 兼容边界、chunk 参数和 multipart 能力；前端据此确认是否支持文件交付以及 multipart 的限制参数 |
...
- `/api/file_policy` 仍会返回 `multipartThresholdBytes`，但它现在是部署策略/兼容性约束字段，不再决定文件写入路径；当前文件交付要求 multipart 对象存储和 `fileRef`。
