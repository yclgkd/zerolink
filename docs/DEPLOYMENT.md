...
ZeroLink 支持对前端构建产物进行 Ed25519 签名，供用户验证完整性。
...
# 将私钥转为 base64（用于 CI Secret）
base64 -i keys/manifest-signing.pem | tr -d '\n'
```
...
# 签名 manifest
MANIFEST_SIGNING_KEY=$(base64 -i keys/manifest-signing.pem | tr -d '\n') \
  pnpm manifest:sign
