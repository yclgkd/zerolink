...
function renderStartupFailureGate(root: HTMLElement): void {
  ...
  card.style.cssText =
    'max-width:640px;width:100%;border:1px solid rgba(249,115,22,0.35);background:rgba(20,20,30,0.72);backdrop-filter:blur(18px);border-radius:1rem;padding:32px;box-shadow:0 24px 80px rgba(0,0,0,0.45);';

  const badge = document.createElement('p');
  badge.style.cssText =
    'margin:0 0 8px 0;color:#f97316;font-size:12px;font-weight:700;letter-spacing:0.35em;text-transform:uppercase;';
  badge.textContent = 'Release Guard';

  const heading = document.createElement('h1');
  heading.style.cssText = 'margin:0 0 12px 0;font-size:28px;font-weight:500;line-height:1.5;';
  heading.textContent = 'Verification Unavailable';

  const body = document.createElement('p');
  body.style.cssText = 'margin:0 0 14px 0;color:#94a3b8;font-size:14px;line-height:1.6;';
  body.textContent = 'ZeroLink could not complete release verification before startup.';
  ...
  note.textContent = 'Do not enter passwords, API keys, or private messages on this page.';

  card.append(badge, heading, body, note);
  section.append(card);
